...
target_material1 = OmniPBR("/World/looks/t1", color=np.array([0.6, 0.7, 1.0]))
target_material2 = OmniPBR("/World/looks/t2", color=np.array([0.6, 0.8, 0.7]))
target_material3 = OmniPBR("/World/looks/t3", color=np.array([0.9, 0.9, 0.7]))
target_material4 = OmniPBR("/World/looks/t4", color=np.array([0.8, 0.7, 0.8]))
...
def main():
    ...
    auto_target_1 = cuboid.VisualCuboid(
        "/World/auto_target_1",
        position=np.array([0.5, -0.5, 0.6]),
        orientation=np.array([0, 1.0, 0, 0.0]),
        visual_material=target_material1,
        color=np.array([0.0, 0.2, 0.2]),
        size=0.05,
        scale=[1.0, 1.0, 1.0],
    )
    auto_target_2 = cuboid.VisualCuboid(
        "/World/auto_target_2",
        position=np.array([0.68, -0.228, 0.185]),
        orientation=np.array([0.6369, 0.6318, 0.2676, 0.6417]),
        visual_material=target_material2,
        color=np.array([0.0, 0, 0.2]),
        size=0.04,
    )
    auto_target_3 = cuboid.VisualCuboid(
        "/World/auto_target_3",
        position=np.array([0.6, 0.6, 0.42]),
        # position=np.array([0.54, 0.55, 0.25]),
        orientation=np.array([0.0, 1.0, 0.0, 0.0]),
        color=np.array([0.2, 0.2, 0]),
        visual_material=target_material3,
        size=0.04,
    )
    auto_target_4 = cuboid.VisualCuboid(
        "/World/auto_target_4",
        position=np.array([0.509, 0.2934, 0.2757]),
        orientation=np.array([0.19, 0.22, 0.53, 0.79]),
        color=np.array([0.2, 0, 0.2]),
        visual_material=target_material4,
        size=0.04,
    )
    ...
    target_list = [auto_target_1, auto_target_2, auto_target_3, auto_target_4]
    ...
    while simulation_app.is_running():
        ...
        if cmd_plan is None and step_index % 10 == 0:
            ...
            auto_target_position, auto_target_orientation = target_list[target_idx].get_world_pose()
            if target_idx == 0:
                target_material1.set_color(np.ravel([0.0, 0.2, 0.6]))  
                target_material2.set_color(np.ravel([0.6, 0.8, 0.7]))  
                target_material3.set_color(np.ravel([0.9, 0.9, 0.7]))   
                target_material4.set_color(np.ravel([0.8, 0.7, 0.8])) 
            elif target_idx == 1:
                target_material1.set_color(np.ravel([0.6, 0.7, 1.0]))
                target_material2.set_color(np.ravel([0.0, 0.2, 0.2]))
                target_material3.set_color(np.ravel([0.9, 0.9, 0.7]))
                target_material4.set_color(np.ravel([0.8, 0.7, 0.8]))
            elif target_idx == 2:
                target_material1.set_color(np.ravel([0.6, 0.7, 1.0]))
                target_material2.set_color(np.ravel([0.6, 0.8, 0.7]))
                target_material3.set_color(np.ravel([0.4, 0.3, 0.0]))
                target_material4.set_color(np.ravel([0.8, 0.7, 0.8]))
            elif target_idx == 3:
                target_material1.set_color(np.ravel([0.6, 0.7, 1.0]))
                target_material2.set_color(np.ravel([0.6, 0.8, 0.7]))
                target_material3.set_color(np.ravel([0.9, 0.9, 0.7]))
                target_material4.set_color(np.ravel([0.4, 0.0, 0.4]))
            # Set EE teleop goals, use cube for simple non-vr init:
            ee_translation_goal = auto_target_position
            ee_orientation_teleop_goal = auto_target_orientation
